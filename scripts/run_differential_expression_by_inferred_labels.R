...
#' 7. Run DESeq2 and write full plus significant DE results.
...
usage <- function() {
  cat(
    paste(
      "Usage:",
      "Rscript scripts/run_differential_expression_by_inferred_labels.R \\",
      "  --counts-path <path> \\",
      "  [--results-path <path>] \\",
      "  [--metadata-path <path>] \\",
...
run_deseq <- function(counts_df, gene_col, labels_dt, id_prefix_length,
  ...
  list(dds = dds, res = res, col_data = design_data, design_formula = design_formula)
}
...
main <- function() {
  ...
  if (length(covariate_cols) == 0) {
    stop("--covariates must include at least one column.")
  }
