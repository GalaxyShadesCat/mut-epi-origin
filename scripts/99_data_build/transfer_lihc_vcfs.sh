...
#   bash scripts/99_data_build/transfer_lihc_vcfs.sh --metadata-csv data/derived/master_metadata.csv --cohort-label nafld
#   bash scripts/99_data_build/transfer_lihc_vcfs.sh --metadata-csv data/derived/master_metadata.csv --cohort-label nafld --required-complete-fields alcohol_status,hbv_status,hcv_status,nafld_status,obesity_class
#   bash scripts/99_data_build/transfer_lihc_vcfs.sh --test --ip 10.0.0.1 --port 22 --username alice --password 'secret'
#   bash scripts/99_data_build/transfer_lihc_vcfs.sh --ip 10.0.0.1 --port 22 --username alice --password 'secret'
...
WGS_TCGA25_ROOT="$(python - <<'PY' "${CONFIG_PATH}" "${PROJECT_ROOT}"
import json
import sys
...
if not config_path.exists():
    print(project_root / "data" / "raw" / "WGS_TCGA25")
    raise SystemExit(0)

cfg = json.loads(config_path.read_text(encoding="utf-8"))
raw_root = str(cfg.get("wgs_tcga25_root", "")).strip()
if not raw_root:
    print(project_root / "data" / "raw" / "WGS_TCGA25")
    raise SystemExit(0)
...
DEST_DIR="${DEST_DIR:-${WGS_TCGA25_ROOT}/AtoL/VCF}"
...
usage() {
    cat <<'EOF'
Usage:
  bash scripts/99_data_build/transfer_lihc_vcfs.sh [--test] [--metadata-csv <path>] [--cohort-label <label>] [--required-complete-fields <csv>] [--ip <ip>] [--port <port>] [--username <name>] [--password <password>]
