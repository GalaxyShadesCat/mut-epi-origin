...
## LIHC SNV mutation table

This directory also contains `lihc_snv_mutation_table.tsv`, a **TCGA-LIHC tumour SNV** table built from per-sample VCF files in this project pipeline.

The file is generated by:
- `scripts/transfer_lihc_vcfs.sh` (builds `data/derived/manifests/lihc_tumour_vcf_candidates.tsv` and transfers candidate VCFs)
- `scripts/build_snv_mutation_table.py` (reads the manifest and writes this SNV table)

Current columns are:
- `chrom`
- `start` (0-based)
- `end` (1-based)
- `sample` (tumour sample submitter identifier)
- `ref`
- `alt`
- `cohort` (set to `LIHC`)
